...
CIDnetwork$methods(
    initialize = function(edge.list,
                          #specs: this should be numbered from 1 to the max edge number. #        sociomatrix,
                          edge.list.rows,
                          n.nodes,
                          node.names = character(), #this should come from the data.
                          intercept = 0,
                          intercept.m = 0,
                          intercept.v = 1000000,
                          residual.variance = 1,
        ...
        outcome.types <- c("binary", "gaussian", "ordinal")
        if (!(class.outcome %in% outcome.types)) {
            error.text <- paste(
                "class.outcome",
                class.outcome,
                "is not supported. Must be one of",
                paste(outcome.types, collapse = ",")
            )
            stop(error.text)
        }
        ...
        if (int.correlation != 0)
            include.reciprocity <- TRUE

        # handles arguments pertaining to sociomatrix input
        # NOTE: lots of conditionals are very confusing, maybe better logic?
        if (missing(edge.list)) {
            ...
            if (missing(is.directed)) {
                    # NOTE: this is really hard to read. should be extracted into
                    #       function.
                    # NOTE: I think there's something to be said about giving the
                    #       user too much flexibility with the input. This is the
                    #       idea behind "tidy data". There's a package called
                    #       tidygraph that standardizes network data in an
                    #       easily accessible dataframe
                e1 <- unique(edge.list)
                ...
                if (nrow(e1) != nrow(e2)) {
                    if (verbose > 0)
                        # message pasts stogether positional arguments with
                        # no separator
                        message(
                            "CIDnetwork: Reciprocal potential edges ",
                            "detected; assuming this is a directed network."
                        )
                    .self$is.directed <<- TRUE
                } else {
                    if (verbose > 0) {
                        message(
                            "CIDnetwork: No reciprocal potential edges ",
                            "detected; assuming this is an undirected network."
                        )
                    }
        ...
        # NOTE: Why not just always make the edge.list.rows?
        ...
        if (length(node.names) != .self$n.nodes) {
            if (verbose > 0)
                message(
                    "Warning:  Length of node.names differs from n.nodes. ",
                    "Using default node names."
                )
        ...
        if (length(components) > 0) {
            ...
            # update the component field after reinitialization
        ...
        # NOTE: can make a debug/verbose print function to get rid of
        #       these if (verbose) conditionals
        if (verbose > 1) {
        ...
        if (reciprocity.present) {
            ...
            reciprocal.match <<- apply(
                X = .self$edge.list,
                MARGIN = 1,
                FUN = function(ee) min(which(.self$edge.list[, 1] == ee[2] & .self$edge.list[, 2] == ee[1]))
            )
        } else {
        ...
        if (length(.self$outcome) != nrow(.self$edge.list))
            stop(
                "In CIDnetwork$initialize: Outcome variable length: ",
                length(.self$outcome),
                "; Edges: ", nrow(.self$edge.list),
                ". Did you forget to add a required variable?"
            )
        ...
        if (class.outcome == "ordinal") {
            ...
            if (any(counts == 0)) {
                stop("The following ordinal categories have no outcomes: ",
                     paste(which(counts == 0) - 1, collapse = " "),
                     "; required non-zero counts for ",
                     paste(0:(.self$ordinal.count-1), collapse = " ")
                )
            }
        }
